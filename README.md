...
> - 所有 `hsl()` / `hsla()` / `rgb()` / `rgba()` 色相统一为 210，饱和度**按亮度分段设上下限**，保留原亮度与 alpha：
>   上限 L≥92 取 48% / L≥80 取 32% / L≥30 取 15% / 更暗取 20%；
>   **下限** L≥92 取 45%（同 Blue Topaz 冷白）/ L≥80 取 30%
> - 只设上限不够：上限是 `min(原值, 上限)`，只会往下压。Primary 亮面原本就是 36~39%，
>   低于上限，调高上限对它们毫无作用——必须用下限把浅色面顶成冷蓝白
> - 中性灰染为同色相的蓝灰；纯白（L≥99）与纯黑（L≤2）不动——给它们染色会让白不再是白，
>   且阴影遮罩本就该保持中性
> - 饱和度用**封顶**而非缩放：原本就淡的保持淡，原本浓的压到上限，
>   于是色调统一、亮度层次不变。深色模式因此自然呈深海军蓝
> - 错误、警告、成功等**语义色保留原色**。色相统一时的跳过规则只认 `--color-red`
>   这类 Obsidian 核心变量名，漏掉了 Primary 的 `--color-l-red-30` 等带层级命名的色阶，
>   故另行恢复 `--background-modifier-error/success/warning` 与 `--text-error/success/warning`。
>   只恢复语义端点、不整条恢复色阶——Primary 把那几条色阶也用在语义之外（如
>   `--inline-code-color` 取 `--color-l-red-10`），整条恢复会让行内代码变红、焦点环变橙
> - **链接作为功能色保留色相**，否则会淹没在蓝调界面里：按类型分三色
>   （双链橙 / 外链松石绿 / 裸网址朱红），未解析双链保持灰。
>   双链取橙是因为它是蓝的补色，在蓝调界面上辨识度最高
> - **高亮五色**按文字样式区分：普通浅蓝 / 粗体深蓝 / 斜体红 / 粗斜体橙 / 删除线柠檬黄；
>   底色与字色的对比度实测 5.64–12.13:1，全部高于 WCAG AA 的 4.5:1
> - **表格表头为强调蓝、白字，正文白底**；**设置页选中项、CTA 与 mod-warning 按钮、
>   开关打开态同为强调蓝**，文字与图标反白
> - **笔记正文底色为纯白**，侧边栏与标签栏保持蓝白
> - **主区活动标签为强调蓝、文字反白**。该标签走
>   `.mod-root .workspace-tabs:not(.mod-stacked)` 下的 `--tab-active-bg`，
>   而该变量侧栏标签也在用，故用定向选择器而非改变量，避免侧栏一起变蓝
